"""Daily Report 회귀 테스트 결과 채점 스크립트.

Ground truth (tests/regression/fixtures/ground_truth_2026_05_15.json)와 rubric
(tests/regression/fixtures/rubric.md)을 기준으로 각 run 출력을 채점한다.

채점은 단순 패턴 매칭(rule-based)으로 진행하여 결정적이고 빠르게 동작한다.
LLM-as-Judge 채점이 필요하면 score_runs_llm.py를 별도로 구현한다.

Usage:
    uv run python tests/regression/scripts/score_runs.py <runs_dir>

Output:
    <runs_dir>/scores.csv     - 각 run의 항목별 점수
    <runs_dir>/summary.md     - 모델별 통계 + Top-N 패턴
"""
