...
from src.application.weekly_summary_use_case import GenerateWeeklySummaryUseCase
from src.domain.models import Report, ReportConfig


class TestGenerateWeeklySummaryUseCase:
    """주간 요약 보고서 생성 유스케이스 테스트"""
    ...
    def use_case(self, mock_report_generator, mock_notifier):
        """테스트용 유스케이스 인스턴스"""
        return GenerateWeeklySummaryUseCase(
    ...
    def test_should_build_title_without_team_prefix(
        ...
        call_args = mock_notifier.send.call_args
        assert call_args[0][0] == "[26.01.27_Weekly]"
